...
module Day07 =
    ...
    open AdventOfCode.NoSpaceLeftOnDevice
    open Xunit
    open FsUnit.Xunit
...
"""
    ...
    [<InlineData("/", 48381165)>]
    let ``2022 - Day 07 - part 1 - total size of directory by path`` (fullPath, expectedSize) =
        exampleInput
        |> directoriesWithSizes
        |> Seq.find (fun (path,_) -> path = fullPath)
        |> snd
        |> should equal expectedSize
        
    [<Fact>]
    let ``2022 - Day 07 - part 1 - example`` () =
        exampleInput
