...
module Day02 =
    ...
    open AdventOfCode.cubeconundrum
    open Xunit
    open FsUnit.Xunit
    open ApprovalTests
...
"""
    ...
    [<Fact>]
    let ``2023 - Day 02 - part 1 - example - parseInput`` () =
        let parsedGames =
            exampleInput
            |> parser.parseInput

        parsedGames |> Array.length |> should equal 5

        Approvals.Verify $"%A{parsedGames}"

    [<Fact>]
    let ``2023 - Day 02 - part 1 - example - possibleGiven`` () =
        exampleInput
        |> possibleGamesGiven { red = 12; green = 13; blue = 14 }
        |> Seq.length
        |> should equal 3

    [<Fact>]
    let ``2023 - Day 02 - part 1 - example`` () =
        exampleInput
        |> sumOfIDsOfPossibleGamesGiven { red = 12; green = 13; blue = 14 }
        |> should equal 8 

    [<Fact>]
    let ``2023 - Day 02 - part 1`` () =
        puzzleInput
        |> sumOfIDsOfPossibleGamesGiven { red = 12; green = 13; blue = 14 }
        |> should equal 2776
